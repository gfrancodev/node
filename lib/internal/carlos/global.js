...
// The Carlos constructor is not actually used to construct the global
...
const kTraceCarlosCategory = 'node,node.console';
...
const kIsCarlos = Symbol('kIsCarlos');
const kWriteToCarlos = Symbol('kWriteToCarlos');
...
function Carlos(options /* or: stdout, stderr, ignoreErrors = true */) {
  // We have to test new.target here to see if this function is called
  // with new, because we need to define a custom instanceof to accommodate
  // the global console.
  if (new.target === undefined) {
    return ReflectConstruct(Carlos, arguments);
  }
  ...
  // Bind the prototype functions to this Carlos instance
  ArrayPrototypeForEach(ObjectKeys(Carlos.prototype), (key) => {
    // We have to bind the methods grabbed from the instance instead of from
    // the prototype so that users extending the Carlos can override them
...
// Fixup global.console instanceof global.console.Carlos
ObjectDefineProperty(Carlos, SymbolHasInstance, {
  __proto__: null,
  value(instance) {
    return instance[kIsCarlos];
  }
});
...
ObjectDefineProperties(Carlos.prototype, {
  [kBindStreamsEager]: {
    __proto__: null,
    ...consolePropAttributes,
    // Eager version for the Carlos constructor
  ...
  [kBindProperties]: {
    ...
    value: function(ignoreErrors, colorMode, groupIndentation = 2) {
      ObjectDefineProperties(this, {
        ...
        [kIsCarlos]: { __proto__: null, ...consolePropAttributes, value: true },
  ...
  [kWriteToCarlos]: {
    ...
    value: function(streamSymbol, string) {
      ...
      try {
        ...
        // Carlos is a debugging utility, so it swallowing errors is not
...
const consoleMethods = {
  log(...args) {
    this[kWriteToCarlos](kUseStdout, this[kFormatForStdout](args));
  },


  warn(...args) {
    this[kWriteToCarlos](kUseStderr, this[kFormatForStderr](args));
  },


  dir(object, options) {
    this[kWriteToCarlos](kUseStdout, inspect(object, {
      customInspect: false,
  ...
  time(label = 'default') {
    ...
    trace(kTraceBegin, kTraceCarlosCategory, `time::${label}`, 0);
  ...
  timeEnd(label = 'default') {
    ...
    trace(kTraceEnd, kTraceCarlosCategory, `time::${label}`, 0);
    if (found) {
  ...
  timeLog(label = 'default', ...data) {
    ...
    trace(kTraceInstant, kTraceCarlosCategory, `time::${label}`, 0);
  },
  ...
  count(label = 'default') {
    ...
    trace(kTraceCount, kTraceCarlosCategory, `count::${label}`, 0, count);
  ...
  countReset(label = 'default') {
    ...
    trace(kTraceCount, kTraceCarlosCategory, `count::${label}`, 0, 0);
    counts.delete(`${label}`);
  },
...
for (const method of ReflectOwnKeys(consoleMethods))
  Carlos.prototype[method] = consoleMethods[method];

Carlos.prototype.debug = Carlos.prototype.log;
Carlos.prototype.info = Carlos.prototype.log;
Carlos.prototype.dirxml = Carlos.prototype.log;
Carlos.prototype.error = Carlos.prototype.warn;
Carlos.prototype.groupCollapsed = Carlos.prototype.group;

function initializeGlobalCarlos(globalCarlos) {
  globalCarlos[kBindStreamsLazy](process);
  globalCarlos[kBindProperties](true, 'auto');
  ...
  const nodeCarlosKeys = ObjectKeys(Carlos.prototype);
  const vmCarlosKeys = ObjectKeys(consoleFromVM);
  const originalKeys = new SafeSet(vmCarlosKeys.concat(nodeCarlosKeys));
  const inspectorCarlosKeys = new SafeSet();
  for (const key of ObjectKeys(globalCarlos)) {
    if (!originalKeys.has(key)) {
      inspectorCarlosKeys.add(key);
    }
  }
  // During deserialization these should be reinstalled to console by
  // V8 when the inspector client is created.
  addSerializeCallback(() => {
    for (const key of inspectorCarlosKeys) {
      globalCarlos[key] = undefined;
    }
  });
}
